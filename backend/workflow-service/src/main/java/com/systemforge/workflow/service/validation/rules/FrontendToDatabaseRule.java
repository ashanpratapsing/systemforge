...
@Component
public class FrontendToDatabaseRule implements GraphValidationRule {
    ...
    public List<ValidationIssue> evaluate(SaveWorkflowRequest graph) {
        ...
        for (SaveWorkflowRequest.EdgeDTO edge : graph.getEdges()) {
            ...
            if (source != null && target != null) {
                if ("frontend".equalsIgnoreCase(source.getType()) && "database".equalsIgnoreCase(target.getType())) {
                    issues.add(new ValidationIssue(
                            source.getId(),
                            edge.getId(),
                            ValidationIssue.Severity.ERROR,
                            "Invalid Architecture Pattern",
                            "Frontend components should not connect directly to Databases. Please route through an API or Backend service."
                    ));
                }
            }
        }
